...
public class NetworkMenu : MonoBehaviour
{
	...
	void CreatePlayer()
	{
		...
		if(Network.isClient)
			GetComponent<NetworkView>().RPC("CheckVersion",RPCMode.Server,version);
	...
	void CheckVersion(int ver, NetworkMessageInfo info)
	{
		...
		if(version != ver)
		{
			GetComponent<NetworkView>().RPC("SendError",info.sender,"Version mismatch: v" + ver.ToString());
			Network.CloseConnection(info.sender,true);
		}
		else
			GetComponent<NetworkView>().RPC("ReqestScore",info.sender);
	}
	
	[RPC]
	void ReqestScore()
	{
		GetComponent<NetScore>().ServAddPlayer(pname,plobj.GetComponent<NetworkView>().viewID);
	}
	
	public void CallError(NetworkPlayer trg, string msg)
	{
		GetComponent<NetworkView>().RPC("SendError",trg,msg);
