...
public class PlayerControl : MonoBehaviour
{
	...
	void Start()
	{
		...
		snd_idx = 0;
		//Water
		water = GameObject.Find("Water").transform.Find("Water").GetComponent<Collider>();
	}
	...
	void FixedUpdate()
	{
		...
		if(grounded)
		{
			...
			{
				GetComponent<AudioSource>().PlayOneShot(snd_jump);
				GetComponent<NetworkView>().RPC("SendJump",RPCMode.Others);
				mdir.y = jumpspeed;
			}
		...
		{
			if(!inwater)
				GetComponent<AudioSource>().PlayOneShot(snd_dirt[snd_idx]);
			else
				GetComponent<AudioSource>().PlayOneShot(snd_water[snd_idx]);
		}
		...
		if(!snd_wait && grounded && move_vlc.magnitude > 3.0f)
		{
			...
			{
				GetComponent<AudioSource>().PlayOneShot(snd_dirt[snd_idx]);
				GetComponent<NetworkView>().RPC("SendDirt",RPCMode.Others,snd_idx);
			}
			else
			{
				GetComponent<AudioSource>().PlayOneShot(snd_water[snd_idx]);
		...
		if(Physics.Raycast(transform.position,-transform.up,out hit,raylen))
		{
			if(hit.collider == water)
			{
				if(!inwater && ctrl.velocity.y < -2.0f)
					GetComponent<NetworkView>().RPC("SendSplash",RPCMode.All,hit.point);
				inwater = true;
			}
			else inwater = false;
		}
	...
	{
		GetComponent<AudioSource>().PlayOneShot(snd_dirt[id]);
	}
	
	[RPC]
	void SendWater(int id)
	{
		GetComponent<AudioSource>().PlayOneShot(snd_water[id]);
	}
